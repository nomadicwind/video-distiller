...
**键盘输入**：`Ctrl+2`、`Alt+Q`、`Shift+3` 等按键组合在录入模式下自动识别，标记标签采用规范序（Ctrl → Alt → Shift），与物理按下顺序无关。该模式使用 `e.code`（物理键位）而非 `e.key`（字形值），所以非 QWERTY 键盘布局亦正确对应数字/字母（如 Dvorak 键盘的物理 Digit0-9、KeyA-Z 位置）。**注**：Cmd/Meta 键不参与组合。

**面板粘滞修饰**：Entry Panel 顶部四个修饰芯片（Ctrl、Alt、Shift、空标记）。点击修饰芯片"着色"（armed 状态），再点击数字/字母基础键帽，将组合记录为单条标记后自动清空已选修饰。反复点击修饰芯片可调整组合，完全不激活键盘即可完成输入——适合需要高精度、低干扰的标注场景。
